...
export class CSVPlayerAnalyzer {
  private pythonScriptPath = path.join(process.cwd(), 'server', 'python', 'enhanced_player_analyzer.py');
  private csvDataPath = path.join(process.cwd(), 'players_data_2025_2026.csv');
  ...
  private async runPythonScript(args: string[]): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      python.stdout.on('data', (data) => {
        console.log(`[CSVAnalyzer] stdout: ${data.toString()}`);
        stdout += data.toString();
      });

      python.stderr.on('data', (data) => {
        console.error(`[CSVAnalyzer] stderr: ${data.toString()}`);
        stderr += data.toString();
      });

      python.on('close', (code) => {
        console.log(`[CSVAnalyzer] Closed with code ${code}`);
        if (code !== 0) {
        ...
        try {
          ...
          console.error(`[CSVAnalyzer] JSON Parse error: ${error.message} - Raw: ${stdout}`);
          reject(new Error(`Failed to parse Python output: ${error.message}`));
        }
      });

      python.on('error', (error) => {
        console.error(`[CSVAnalyzer] Spawn error: ${error.message}`);
