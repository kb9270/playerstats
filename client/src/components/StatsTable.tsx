...
export default function StatsTable({ playerIds }: StatsTableProps) {
  ...
  const playerQueryResults = useQueries({
    queries: sortedPlayerIds.map(id => ({
  ...
  const statsQueryResults = useQueries({
    queries: sortedPlayerIds.map(id => ({
  ...
  const playerQueries = sortedPlayerIds.map((id, index) => ({
    id,
    playerQuery: playerQueryResults[index],
    statsQuery: statsQueryResults[index]
  }));
          ...
          <TableRow className="border-b border-gray-700">
            ...
            {playerQueries.map(({ playerQuery }, index) => (
              <TableHead key={index} className={`text-center py-4 px-2 font-medium ${playerColors[index]}`}>
                {(playerQuery.data as any)?.name || `Joueur ${index + 1}`}
              </TableHead>
            ))}
          </TableRow>
        </TableHeader>
          ...
          {statCategories.map((stat) => (
            <TableRow key={stat.key} className="hover:bg-stats-dark/30 border-b border-gray-700/50">
              ...
              {playerQueries.map(({ statsQuery }, index) => {
                const statsArray = statsQuery.data as any[] | undefined;
                const statsData = statsArray?.[0]; // Get most recent stats
                const statValue = statsData?.[stat.key] as number | null;
                const minutes = statsData?.minutes as number | null;
