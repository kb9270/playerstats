...
import BasicAutoComparison from "@/components/BasicAutoComparison";
...
export default function PlayerDetailedProfile() {
  ...
  const generateComparatif = async () => {
    ...
    try {
      const response = await fetch(`/api/players/${encodeURIComponent(decodedPlayerName)}/analysis`);
      
      const data = await response.json();
      setComparatif(data.resume_detaille || data.analysis?.resume_detaille || "Aucune réponse générée.");
    } catch (error) {
        ...
        <Card className="bg-gradient-to-r from-purple-900/40 to-pink-900/40 border-purple-500/30">
          <CardContent className="p-6">
            ...
            {autoComparisonData?.success ? (
              <BasicAutoComparison data={autoComparisonData} />
            ) : loadingAutoComparison ? (
